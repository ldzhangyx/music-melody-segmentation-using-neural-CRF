...
class CrossValidator:
    ...
    def compute(self):
        n_iter = 0
        cur_model, train_loader = self.data_prep()
        ...
        for j in range(self.epochs):
            ...
            name = 'cnncrf_linear_train_epoch' + str(j)+ str(self.augment_data_flag)+'.pt'
            torch.save(cur_model.state_dict(), name)
            print("completed: ", float(j) / float(self.epochs))
            sys.stdout.flush()
            _p, _r, _ = self.validation(cur_model)
            print(f'validation result: p:{_p}, r:{_r}')
            if early_stop:
                break

        return self.validation(cur_model)

    def load_and_valid(self):
        cur_model, _ = self.data_prep()
        self.precision_history, self.recall_history, self.loss_history = self.validation(cur_model)
    ...
    def data_prep(self):
        ...
        return cur_model, train_loader

    def validation(self, cur_model):
        # test
        p0t1 = 0
        p0t0 = 0
        p1t1 = 0
...
class CNNCRF(nn.Module):
    ...
    def forward(self, sentence):  # dont confuse this with _forward_alg above.
        ...
        cnn_feats = self._get_cnn_features(sentence)

        # Find the best path, given the features.
        score, tag_seq = self._viterbi_decode(cnn_feats)
        return score, tag_seq

print('I am in!')
model = CNNCRF(input_dim, hidden_dim, output_size, START_TAG, STOP_TAG, BATCH_SIZE)
# model.load_state_dict(torch.load("/gpfsnyu/home/yz6492/melody_seg/cnncrf_linear_train_epoch7.pt"))
total_num = sum(p.numel() for p in model.parameters())
trainable_num = sum(p.numel() for p in model.parameters() if p.requires_grad)
print({'Total': total_num, 'Trainable': trainable_num})
sys.stdout.flush()
validator = CrossValidator(model, compute_acc=compute_acc, partition = 1, batch_size=BATCH_SIZE, epochs=15, lr=1e-2, augment_data=True)
precision, recall, loss = validator.compute()
# print(precision, recall, loss)
out_f = open("/gpfsnyu/home/yz6492/melody_seg/data/essen_cnncrf_linear_report.pkl", "wb")
