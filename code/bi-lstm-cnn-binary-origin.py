            ...
            for num, (train_X, train_Y)in enumerate(train_loader):
                ...
                if (num) % self.print_every == 0:
                    ...
                    if(score > self.best_acc):
                        torch.save(cur_model.state_dict(), 'Bi-LSTM-CNN_best.pt')
                        self.best_acc = score
                    print("best_score:", self.best_acc)
                    sys.stdout.flush()

            scheduler.step()
            name = 'lstmcnn_binary_train_epoch' + str(j) + '.pt'
...
# precision, recall, fscore = cv.compute()
#
...
if is_validate:
    ...
    model.load_state_dict(torch.load("/gpfsnyu/home/yz6492/melody_seg/lstmcnn_binary_train_epoch4.pt"))
